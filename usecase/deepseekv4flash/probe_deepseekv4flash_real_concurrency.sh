...
python probe_sglang_real_concurrency.py \
  ...
  --server-log "${SERVER_LOG:-/data/lin/modelserver/logs/deepseekv4flash_rank0.log}" \
  --results-dir "${RESULTS_DIR:-results/deepseekv4flash_real_concurrency_$(date +%Y%m%d_%H%M%S)}" \
  --workloads "${WORKLOADS:-8192:1024,16384:1024,32768:1024,65536:1024,131072:1024,262144:1024,393216:1024,524288:1024,786432:1024,983040:512}" \
